...
public class OptionalEx<T> {
     ...
     *
     * @implNote Generally only one empty instance, {@link OptionalEx#EMPTY},
     * should exist per VM.
     */
    private OptionalEx() {
        this.value = null;
    ...
    public static <T> OptionalEx<T> OptionalEx(Optional<T> optional) {
        return ofNullable(optional.get());
    }
    /**
    ...
    public<U> OptionalEx<U> uncheckedMap(CheckedFunction<? super T, ? extends U> mapper) {
        Objects.requireNonNull(mapper);
        if (!isPresent())
            return empty();
        else {
